...
function AjouterPR({ pr }) {
    const form = useForm({
        defaultValues: {
            numberTaxPayer: pr?.numberTaxPayer
        }
    });
    ...
    return (
        <>
            <div className="w-sm-100  w-75 mx-auto my-3 shadow">
                <h3 className="text-center rounded-top p-3 bg-info">Ajouter un permis de recherche</h3>
                <form action="" method="post" className="form p-4" onSubmit={(e) => handleSubmit(e)}>
                    ...
                    <h3 className="w-auto p-2 bg-light" onClick={handleShowLicence}>Permis
                        <span style={{ float: "right" }}>
                            {showLicence ? "-" : "+"}
                        </span>
                    </h3>
